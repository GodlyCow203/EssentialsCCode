...
public class ShopSoundManager {
    ...
    public ShopSoundManager(EssentialsC plugin) {
        this.plugin = plugin;
    }
    ...
    public void playSale(Player player) {
    ...
    public void playInsufficientFunds(Player player) {
        player.playSound(player.getLocation(), Sound.ENTITY_VILLAGER_NO, 0.6f, 1.0f);
    }

    public void playInventoryFull(Player player) {
        player.playSound(player.getLocation(), Sound.BLOCK_ANVIL_LAND, 0.5f, 1.0f);
    }

    public void playNoPermission(Player player) {
